...
def fetch_tracks(url):
    ...
    elif "youtube.com" in url or "youtu.be" in url or "soundcloud.com" in url:
        return get_youtube_tracks(url)

    raise ValueError("URL must be a supported Spotify, YouTube, or SoundCloud link.")
...
def get_lrclib_lyrics(track_name, artist_name, album_name=None):
    ...
    album_cover_url = ""
    try:
        sources = album_union.get("coverArt", {}).get("sources", [])
        if sources:
            sources.sort(key=lambda x: x.get("width", 0), reverse=True)
            album_cover_url = sources[0].get("url")
    except Exception:
    ...
    tracks = []
    items = album_union.get("tracksV2", {}).get("items", [])
    for item in items:
        track_data = item.get("track", {})
        if not track_data:
            continue
...
def get_single_track(url):
    ...
    if album_match:
        try:
            album_url = album_match.group(0).split('content="')[1].rstrip('"')
            album_tracks = get_album_tracks(album_url)
            if album_tracks:
                album_name = album_tracks[0]["album"]
        except Exception:
            pass
